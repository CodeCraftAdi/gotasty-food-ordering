...
import Plus from "@/app/components/layout/icons/Plus";
  ...
  useEffect(() => {
    fetch('/api/categories').then(res => {
      res.json().then(categories => {
        setCategories(categories); if (!category && categories.length > 0) { setCategory(categories[0]._id);  }
      });
    });
  }, [category]);
      ...
      <div
        ...
        <div className="grow">
          ...
          <MenuItemPriceProps name={'Extra ingredients'}
            addLabel={'Add ingregients prices'}
